...
// Render an OBJMesh entity (flat shaded or per-face textured)
                            ...
                            int *clip_trivial)
{
    ...
    for (int i = 0; i < m->face_count; i++)
    {
        ...
        bool has_texture = face.texture_id >= 0 && face.texture_id < m->texture_count && m->textures != NULL;

        if (has_texture)
        {
            const Texture *tex = &m->textures[face.texture_id];
            float u0 = m->vertices[idx[0]].u;
            float v0 = m->vertices[idx[0]].v;
            float u1 = m->vertices[idx[1]].u;
            float v1 = m->vertices[idx[1]].v;
            float u2 = m->vertices[idx[2]].u;
            float v2 = m->vertices[idx[2]].v;
